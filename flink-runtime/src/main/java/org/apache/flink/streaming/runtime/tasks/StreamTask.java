...
import org.apache.flink.extensions.controller.EnableDisableLookaheadControlMessage;
import org.apache.flink.extensions.controller.GetSelectedLookaheadsControlMessage;
...
public abstract class StreamTask<OUT, OP extends StreamOperator<OUT>>
    ...
    @Override
    public int sendControlGetLookahead(ControlMessage controlMessage) {
        if (controlMessage instanceof GetSelectedLookaheadsControlMessage) {
            LOG.debug("Received control message for reporting the selected lookahead - "
                    + getEnvironment().getTaskInfo().getTaskName()
                    + "-" + getEnvironment().getTaskInfo().getIndexOfThisSubtask());
            StatefulOperatorWithPrefetching statefulOp = getStatefulOperatorWithPrefetching();
            if (statefulOp != null) {
                int selectedLookahead = statefulOp.getSelectedLookaheadID();
    ...
    public void sendControl(ControlMessage controlMessage) {
        if (controlMessage instanceof EnableDisableLookaheadControlMessage) {
            LOG.debug("Received control message for changing the state of the lookahead - "
                    + getEnvironment().getTaskInfo().getTaskName()
                    + "-" + getEnvironment().getTaskInfo().getIndexOfThisSubtask());
            LookaheadOperator lookaheadOperator = getLookaheadOperator();
            if (lookaheadOperator != null) {
                lookaheadOperator.setState(((EnableDisableLookaheadControlMessage)controlMessage).active);
            }
        }
        else {
            throw new UnsupportedOperationException("Control message type not supported");
    ...
    private StatefulOperatorWithPrefetching getStatefulOperatorWithPrefetching() {
        for (StreamOperatorWrapper operator : operatorChain.getAllOperators()){
            if (operator.getStreamOperator() instanceof AbstractUdfStreamOperator) {
                if (((AbstractUdfStreamOperator) operator.getStreamOperator()).getUserFunction()
                        instanceof StatefulOperatorWithPrefetching) {
                    return (StatefulOperatorWithPrefetching) ((AbstractUdfStreamOperator) operator.getStreamOperator()).getUserFunction();
                }
            }
        }
        return null;
    ...
    private LookaheadOperator getLookaheadOperator() {
        for (StreamOperatorWrapper operator : operatorChain.getAllOperators()){
            if (operator.getStreamOperator() instanceof AbstractUdfStreamOperator) {
                if (((AbstractUdfStreamOperator) operator.getStreamOperator()).getUserFunction()
                        instanceof LookaheadOperator) {
                    return (LookaheadOperator) ((AbstractUdfStreamOperator) operator.getStreamOperator()).getUserFunction();
                }
            }
        }
        return null;
